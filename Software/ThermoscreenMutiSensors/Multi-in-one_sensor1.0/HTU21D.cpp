...
float HTU21D::readHumidity(void)
{
	...
	SHT21Wire.beginTransmission(HTDU21D_ADDRESS);
	SHT21Wire.write(TRIGGER_HUMD_MEASURE_NOHOLD); //Measure humidity with no bus holding
	SHT21Wire.endTransmission();

	//Hang out while measurement is taken. 50mS max, page 4 of datasheet.
	delay(55);

	//Comes back in three bytes, data(MSB) / data(LSB) / Checksum
	SHT21Wire.requestFrom(HTDU21D_ADDRESS, 3);

	//Wait for data to become available
	int counter = 0;
	while(SHT21Wire.available() < 3)
	{
		counter++;
		delay(1);
...
float HTU21D::readTemperature(void)
{
	...
	SHT21Wire.beginTransmission(HTDU21D_ADDRESS);
	SHT21Wire.write(TRIGGER_TEMP_MEASURE_NOHOLD);
	SHT21Wire.endTransmission();
...
void HTU21D::setResolution(byte resolution)
{
  ...
  SHT21Wire.beginTransmission(HTDU21D_ADDRESS);
  SHT21Wire.write(WRITE_USER_REG); //Write to the user register
  SHT21Wire.write(userRegister); //Write the new resolution bits
  SHT21Wire.endTransmission();
}
...
byte HTU21D::read_user_register(void)
{
  ...
  SHT21Wire.beginTransmission(HTDU21D_ADDRESS);
  SHT21Wire.write(READ_USER_REG); //Read the user register
  SHT21Wire.endTransmission();
  
  //Read result
  SHT21Wire.requestFrom(HTDU21D_ADDRESS, 1);
  
  userRegister = SHT21Wire.read();

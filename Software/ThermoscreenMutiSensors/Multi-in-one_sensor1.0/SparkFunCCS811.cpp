...
CCS811Core::status CCS811Core::beginCore(void)
{
	...
	while ( CWire.available() ) //Clear wire as a precaution
	{
		CWire.read();
	}
...
CCS811Core::status CCS811Core::readRegister(uint8_t offset, uint8_t* outputPointer)
{
	...
	uint8_t numBytes = 1;
	CCS811Core::status returnError = SENSOR_SUCCESS;
	CWire.beginTransmission(I2CAddress);
	CWire.write(offset);
	if( CWire.endTransmission() != 0 )
	{
		returnError = SENSOR_I2C_ERROR;
	}
	CWire.requestFrom(I2CAddress, numBytes);
	while ( CWire.available() ) // slave may send less than requested
	{
		result = CWire.read(); // receive a byte as a proper uint8_t
	}
...
CCS811Core::status CCS811Core::multiReadRegister(uint8_t offset, uint8_t *outputPointer, uint8_t length)
{
	...
	else  //OK, all worked, keep going
	{
		...
		CWire.requestFrom(I2CAddress, length);
		while ( (CWire.available()) && (i < length))  // slave may send less than requested
		{
			c = CWire.read(); // receive a byte as character
			*outputPointer = c;
			outputPointer++;
			i++;
		}
		//dump extra
		while(CWire.available())
		{
			CWire.read();
		}
	}
...
CCS811Core::status CCS811Core::multiWriteRegister(uint8_t offset, uint8_t *inputPointer, uint8_t length)
{
	...
	{
		CWire.write(*inputPointer); // receive a byte as character
		inputPointer++;
		i++;
	}
	if( CWire.endTransmission() != 0 )
